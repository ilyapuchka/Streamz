...
open class OAuthSwiftHTTPRequest: NSObject, OAuthSwiftRequestHandle {
    ...
    func start(success: SuccessHandler?, failure: FailureHandler?) {
        ...
        OAuthSwiftHTTPRequest.executionContext {
            ...
            if self.session.delegate != nil {
                ...
                self.task = self.session.dataTask(with: usedRequest, completionHandler: { data, resp, error in
                    self.completeRequest(successHandler: successHandler, failureHandler: failureHandler, data: data, response: resp, error: error)
                })
            }
            self.task?.resume()
    ...
    public func completeRequest(successHandler: SuccessHandler?, failureHandler: FailureHandler?, data: Data?, response resp: URLResponse?, error: Error?) {
        #if os(iOS)
            #if !OAUTH_APP_EXTENSIONS
        ...
        if let error = error {
            var oauthError: OAuthSwiftError = .requestError(error: error, request: self.request!)
        ...
        guard let response = resp as? HTTPURLResponse, let responseData = data else {
            ...
            failureHandler?(.requestError(error:error, request: self.request!))
            return
        }
        ...
        // MARK: success
        successHandler?(OAuthSwiftResponse(data: responseData, response: response, request: self.request))
    }
